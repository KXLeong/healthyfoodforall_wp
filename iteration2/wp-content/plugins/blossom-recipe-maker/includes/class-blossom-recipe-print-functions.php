	...
	function br_recipe_call_to_action_buttons($post_id)
	{
		...
		<div>
			<span class="blossom-recipe-print">
				...
				?>
				<a target="_blank" rel="nofollow" href="<?php echo add_query_arg( 'print', $post_id, $recipe_post_url );?>" class="br_recipe_print_button" data-br-recipe-id="<?php echo esc_attr($post_id) ;?>">
					<?php
					echo (isset($options['print_btn_txt']) && ($options['print_btn_txt'] !='')) ? __(esc_html($options['print_btn_txt']), 'blossom-recipe-maker') : __('Print', 'blossom-recipe-maker');
					?>
				</a>
