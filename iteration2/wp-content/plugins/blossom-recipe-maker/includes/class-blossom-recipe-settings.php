	...
	{ 
		...
		if( isset( $_GET['settings-updated'] ) && $_GET['settings-updated'] ==true )
		{
			?>
                ...
                <p><strong><?php _e('Your settings have been saved.','blossom-recipe-maker');?></strong></p><button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php _e('Dismiss this notice.','blossom-recipe-maker');?></span></button>
            	</div>
            <?php
		}
		?>
		...
		<div id="blossom-recipe-settings-navigation">
			...
			$settings_tab = $this->br_recipe_settings_options();
				...
				foreach ($settings_tab as $key => $value)
				{
		        	...
		        	<a <?php if($count==0){ ?>class="nav-tab nav-tab-active"<?php } else{ ?>class="nav-tab" <?php } ?> href="javascript:;"><?php echo esc_attr_e($tab_label, 'blossom-recipe-maker');?></a>
