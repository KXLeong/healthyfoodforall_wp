...
class Blossom_Recipe_Maker_Admin {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( $this->plugin_name.'ingredient-functions-handler', 'RecipeIngredients', array(
		...
		wp_localize_script( $this->plugin_name.'instruction-functions-handler', 'RecipeInstructions', array(
		...
		wp_localize_script( $this->plugin_name.'recipe-gallery-metabox', 'RecipeGallery',
			 array(
	...
	$args = array(
		...
		'description'           => __( '', 'blossom-recipe-maker' ),
		'labels'                => $labels,
		...
		'rewrite' 			    => array('slug' => $permalink['blossom_recipe_base'], 'with_front' => true),
	);
	...
	function brm_create_categories_type_taxonomies() {
		...
		// Add new taxonomy, make it hierarchical
				...
				$args = array(
					...
					'show_in_nav_menus' => true,
					'rewrite'           => array('slug' => $permalink['blossom_recipe_category_base'], 'hierarchical' => true),
				);
	...
	function brm_create_cuisine_type_taxonomies() {
		...
		// Add new taxonomy, make it hierarchical
				...
				$args = array(
					...
					'show_admin_column' => true,
					'show_in_nav_menus' => true,
					'rewrite'           => array('slug' => $permalink['blossom_recipe_cuisine_base'], 'hierarchical' => true),
				);
	...
	function brm_create_cooking_method_type_taxonomies() {
		...
		// Add new taxonomy, make it hierarchical
				...
				$args = array(
					...
					'show_admin_column' => true,
					'show_in_nav_menus' => true,
					'rewrite'           => array('slug' => $permalink['blossom_recipe_cooking_method_base'], 'hierarchical' => true),
				);
	...
	function brm_create_recipe_tags_taxonomies() {
		...
		// Add new taxonomy, make it hierarchical
				...
				$args = array(
					...
					'show_admin_column' => true,
					'show_in_nav_menus' => true,
					'rewrite'           => array('slug' => $permalink['blossom_recipe_tags_base'], 'hierarchical' => false),
				);
    ...
    */
	...
	function filter_recipes_by_taxonomies($post_type, $which)
	{
		...
		foreach ( $taxonomies as $taxonomy_slug ) {
			...
			echo '<option value="">' . sprintf( esc_html__( 'All %s', 'blossom-recipe-maker' ), $taxonomy_name ) . '</option>';
	...
	function brm_recipe_tax_terms(){
		...
		?>
			<label for="brm-categories-select-<?php echo $ran;?>"><?php esc_html_e( 'Choose Terms: (Press Ctrl to select Multiple Terms)', 'blossom-recipe-maker' ); ?></label>
			...
			{
				?>
				<select name="" class="brm-cat-select brm-categories-select-<?php echo $ran;?>" id="brm-categories-select-<?php echo $ran;?>" multiple style="width:350px;" tabindex="4">
				  	<?php
	...
	function brm_recipe_slider_tax_terms(){
		...
		?>
			...
			<?php $terms = get_terms( array(
				...
				{
					?>
					<select name="" class="brm-terms-select brm-categories-term-select-<?php echo $ran;?>" id="brm-categories-term-select-<?php echo $ran;?>" style="width:100%;">
					  	<?php
					  	$categories = get_categories('taxonomy='.$taxonomy);
