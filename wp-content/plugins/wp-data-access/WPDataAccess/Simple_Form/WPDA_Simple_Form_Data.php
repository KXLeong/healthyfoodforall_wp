...
namespace WPDataAccess\Simple_Form {
	...
	class WPDA_Simple_Form_Data {
		...
		public function get_row( $auto_increment_value, $wpda_err ) {
			...
			$table_columns = array(); // Get all table columns.
			foreach ( $this->wpda_list_columns->get_table_columns() as $column ) {
			...
			if ( $use_primary_key ) {
				foreach ( $this->wpda_list_columns->get_table_primary_key() as $pk_column ) {
					...
					$where .= $wpdadb->prepare( $where_current, $pkvalue ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
				}
			} else {
			...
			$result = $wpdadb->get_results( $query, 'ARRAY_A' ); // phpcs:ignore Standard.Category.SniffName.ErrorCode
		...
		public function set_row() {
			...
			if ( null === $column_values_to_be_updated ) {
				// Nothing to update.
				if ( $_REQUEST['wpda_message'] && '' !== $_REQUEST['wpda_message'] ) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
