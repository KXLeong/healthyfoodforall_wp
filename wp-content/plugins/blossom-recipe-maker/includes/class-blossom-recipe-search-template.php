...
class Blossom_Recipe_Maker_Search_Template{
	...
	{	
		...
		?>
		<h4 class="br-search-title"><?php _e('Search for a Recipe','blossom-recipe-maker');?></h4>

		<form method="get" action='<?php echo esc_url(get_permalink($pageID));?>' id="blossom-recipe-search-form">
			<div class="recipe-search-form">
				<input type="hidden" name="recipe-search-nonce" value="<?php echo $nonce;?>">

				<div class='recipe-search-field recipe-keyword'>
					<h4><?php _e('Keyword', 'blossom-recipe-maker');?></h4>
					<input class="search-keyword" placeholder="<?php _e('Search …', 'blossom-recipe-maker');?>" value="<?php echo isset($_GET['search']) ? $_GET['search'] : '' ?>" name="search" id="search" type="text">
				</div>
	            <?php 
	            ...
	            foreach($categories as $category){
	                if($category->count > 0){
	                    $select.= "<option value='".$category->slug."' ".( isset($_GET['recipe-category']) && $category->slug == $_GET['recipe-category'] ? 'selected = "selected"' : '')  .">".$category->name."</option>";
	                }
	            }
	            ...
	            foreach($cuisines as $cuisine){
	                if($cuisine->count > 0){
	                	$select.= "<option value='".$cuisine->slug."' ".( isset($_GET['recipe-cuisine']) && $cuisine->slug == $_GET['recipe-cuisine'] ? 'selected="selected"' : '' ) .">".$cuisine->name."</option>";
	                }
	            }
	            ...
	            foreach($cooking_methods as $cooking_method){
	                if($cooking_method->count > 0){
	                	$select.= "<option value='".$cooking_method->slug."' ".( isset($_GET['recipe-cooking-method']) && $cooking_method->slug == $_GET['recipe-cooking-method'] ? 'selected="selected"' : '' ) .">".$cooking_method->name."</option>";
	                }
	            }
	            ...
	            foreach($tags as $tag){
	                if($tag->count > 0){
	                	$select.= "<option value='".$tag->slug."' ".( isset($_GET['recipe-tag']) && ($tag->slug == $_GET['recipe-tag']) ? 'selected="selected"' : '' ) .">".$tag->name."</option>";
	                }
	            }
	            ...
	            <div class='recipe-search-field-submit'>
	               <input type="submit" name="recipe_search" value="<?php _e( 'Search','blossom-recipe-maker');?>">
	            </div>
