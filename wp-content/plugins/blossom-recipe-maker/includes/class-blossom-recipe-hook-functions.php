...
class Blossom_Recipe_Maker_Hook_Functions {
	...
	function br_recipe_rating($post_id)
	{
		...
		if( isset($recipe['details']) && !empty($filteredrecipe) && $rating != 0 )
		{
	        ...
	        <div class="brm-comment-rating">
	        	<span class="rating-title"><?php _e( 'Ratings: ', 'blossom-recipe-maker' ); ?></span>
				<span class="brm-rating-stars"><?php
					for ( $i = 1; $i <= 5; $i++ ) {
						...
						if ( $i <= $rating ) {
								...
								echo apply_filters( 'brm_comment_rating_star_full_icon', $star_icon );
						} else {
								ob_start();
	...
	{        
		...
		if(isset($recipe['details']) && !empty($filteredrecipe)) 
		{
			...
			<div id="br-recipe-category-links">
				...
				_e('Author: ', 'blossom-recipe-maker');
				...
				if(has_term('', 'recipe-category', $post_id))
				{
					...
					echo get_the_term_list( $post_id, 'recipe-category', __('Category: ', 'blossom-recipe-maker'), ', ' );
					echo '</span></div>';
				...
				if(has_term('', 'recipe-cuisine', $post_id))
				{
					...
					echo get_the_term_list( $post_id, 'recipe-cuisine', __('Cuisine: ', 'blossom-recipe-maker'), ', ' );
					echo '</span></div>';
				}
				...
				if(has_term('', 'recipe-cooking-method', $post_id))
				{
					...
					echo get_the_term_list( $post_id, 'recipe-cooking-method', __('Cooking Method: ', 'blossom-recipe-maker'), ', ' );
					echo '</span></div>';
				}
	...
	public function calculate_time($name, $mins_time)
	{
		...
		echo '<span class="recipe_' .$name.'">';		

		if(isset($hours) && $hours != 0){
			echo esc_html($hours); 
			_e( ' Hr ', 'blossom-recipe-maker');
			
		}
		if(isset($minutes) && $minutes != 0){
			echo esc_html($minutes) ; 
			_e( ' Mins', 'blossom-recipe-maker');
			...
			<div id="br-recipe-details">
				...
				if(isset($details['details']['servings']) && !empty($details['details']['servings']))
				{
					...
					<span class="br_recipe_servings">
						...
						<?php _e( 'Yields: ', 'blossom-recipe-maker' ) ?>

						<?php echo '<span class="recipe_servings">' . esc_html( $details['details']['servings']) ;

						echo ($details['details']['servings'] == 1) ?  __(' Serving', 'blossom-recipe-maker') :					
						__(' Servings', 'blossom-recipe-maker');
						echo '</span>';						
				...
				if(isset($details['details']['prep_time']) && !empty($details['details']['prep_time'] ))
				{
					...
					<span class="br_recipe_prep_time">
						...
						<?php _e( 'Prep Time: ', 'blossom-recipe-maker' ); ?>
						<?php
						$name ='prep_time';
				...
				if(isset($details['details']['cook_time']) && !empty($details['details']['cook_time']))
				{
					...
					<span class="br_recipe_cook_time">
						...
						<?php _e( 'Cook Time: ', 'blossom-recipe-maker' ) ?>
				...
				if(isset($details['details']['total_time']) && !empty($details['details']['total_time']))
				{
					...
					<span class="br_recipe_total_time">
						...
						<?php _e( 'Total Time: ', 'blossom-recipe-maker' ) ?>
	...
	function br_recipe_gallery($post_id)
	{
		...
		if ( isset($gallery['enable']) && !empty($gallery['video_url']) || isset($gallery['enable']) && !empty($gallery[0]) )
		{
			...
			<div class="feat-images-container owl-carousel owl-theme" id="recipe-gallery">
				...
				if(isset($gallery['video_url']) && !empty($gallery['video_url']))
				{
					...
					$video = $oembed->fetch($provider, $url, array('width' => 1170, 'height' => 650));
				...
				if(!empty($gallery[0]))
				{
					if ($gallery) : unset($gallery['enable'], $gallery['video_url']);
		...
		elseif ( isset($options['feat_image']) && has_post_thumbnail($post_id) )
		{
			...
			<div class ="feat-image">
				...
				echo wp_get_attachment_image ( get_post_thumbnail_id($post_id),  $img_size );
				?>
			</div>
		...
		<div id="br-recipe-ingredients">
			...
			if(isset($ingredients['ingredient']) && !empty($filteredingredients)) 
			{
				...
				<div class="br-ingredient-wrap">
					<h4><?php _e( 'Ingredients', 'blossom-recipe-maker' ) ?></h4>
					...
					if(isset($ingredients['details']['servings']) && !empty($ingredients['details']['servings']))
					{
						...
						<div class="br-progressbar-wrap">
							<div id="br_ingredients_counter">
								<?php echo '<span class="ingredient_checked" data-post="'.$post_id.'">' . __('0' , 'blossom-recipe-maker') . '</span>';
								echo '<span class="ingredient_total">' .__( '/', 'blossom-recipe-maker' ) . esc_html( $total_ingredients);
								echo __( ' Ingredients', 'blossom-recipe-maker' ) .'</span>';
								?>
							</div>

							<div class="ingredient-progressbar-container">
								<div class="ingredient-progressbar-bar" data-post="<?php echo $post_id;?>"></div>
							</div>
						</div>
						<?php
					}
				...
				if(!empty($ingredients['details']['servings']	&&  isset($options['adjust_servings']) && $options['adjust_servings'] == 1))
				{
					...
					<div id="adjust_recipe_servings">
						...
						<?php _e( 'Adjust Servings', 'blossom-recipe-maker' ) ?>

						<input min="1" class="br_adjust-recipe-servings" data-original="<?php echo esc_attr($ingredients['details']['servings']);?>" data-post="<?php echo $post_id;?>" value="<?php echo esc_attr($ingredients['details']['servings']);?>" type="number">
					</div>
					<?php
				}
				?>
					...
					<ul>
						...
						foreach( $ingredients['ingredient'] as $ingredient ) 
						{
							...
							elseif(isset($ingredient['ingredient']) && !empty($ingredient['ingredient']))
							{
								...
								?>
								<li>
									<input type="checkbox" id="ingre-counter-wrap-<?php echo $ran;?>" class="ingredients_checkcounter" data-ingre="<?php echo $abc;?>" data-post="<?php echo $post_id;?>">

									<label for="ingre-counter-wrap-<?php echo $ran;?>" class="single_ingredient" id="ingredient_<?php echo $abc ;?>" data-post="<?php echo $post_id;?>">

										<?php echo '<span class="ingredient_quantity" data-original="'.esc_html( $ingredient['quantity']).'" data-post="'.$post_id.'">' . esc_html( $ingredient['quantity']) .'</span>&nbsp;';
		...
		<div id="br-recipe-instructions">
			...
			if(isset($instructions['instructions']) && (!empty($filtereddescription)|| !empty($filteredimage))) 
			{
				...
				<div class="br-instruction-wrap">
					<h4><?php _e( 'Instructions', 'blossom-recipe-maker' ) ?></h4>

					<div class="br-progressbar-wrap">
						<div id="br_instructions_counter">
							<?php echo '<span class="instructions_checked" data-post="'.$post_id.'">' . __('0' , 'blossom-recipe-maker') .'</span>';
							echo '<span class="instructions_total">' . __( '/', 'blossom-recipe-maker' ) . esc_html( $total_instructions);
							echo __( ' Instructions', 'blossom-recipe-maker' ) .'</span>';
							?>
						</div>

						<div class="instruction-progressbar-container">
							<div class="instruction-progressbar-bar" data-post="<?php echo $post_id;?>"></div>
						</div>
					</div>
					...
					<ul>
						...
						foreach( $instructions['instructions'] as $instruction ) 
						{
							...
							elseif(!empty($instruction['description']) || !empty($instruction['image']))
							{
								...
								?>
								<li>
									<input type="checkbox" id="instr-counter-wrap-<?php echo $ran;?>" class="instructions_checkcounter" data-count="<?php echo $count;?>" data-post="<?php echo $post_id;?>">

									<?php echo '<label for="instr-counter-wrap-'.$ran.'" class="instruction_step" id="step_' .$count .'" data-post="'.$post_id.'">' . __('Step ' , 'blossom-recipe-maker') .  esc_html($count) .'</label>'; ?> 
	...
	{ ?>
		<div id="br-recipe-notes">
			...
			{
				?>
				<h4><?php _e( 'Notes', 'blossom-recipe-maker' ) ?></h4>

				<?php 
				$notes = apply_filters( 'br_recipe', $notes['notes'] );
				echo '<span class="recipe_notes">' .  wpautop( html_entity_decode( $notes, 3,'UTF-8' ) ) .'</span>';
				// echo '<span class="recipe_notes">' . wpautop( wp_kses_post($notes) ) .'</span>';
	...
	function br_recipe_post_tags($post_id){
		...
		if(has_term('', 'recipe-tag', $post_id))
		{
			...
			<div class="recipe-tags">
				<h4><?php _e('Tags', 'blossom-recipe-maker');?></h4>
