...
class Brm_Recipe_Categories extends WP_Widget {
	...
	public function widget( $args, $instance ) {
			...
			foreach ($cats as $key => $value) 		
			{
				...
				if ( ! empty( $category ) && ! is_wp_error( $category ))
				{
					...
					if( isset($img) && is_array($img) && isset($img[0]) && $img[0]!='' )
					{
		                ...
		                echo '<li style="background-image: url('.$url1.')">';
						echo '<a '.$target.' href="'. esc_url(get_term_link( $id )) .'"><span class="cat-title">'.$name.'</span>';
						if( $count > 0 ) {
							echo '<span class="post-count">'.esc_html($count).__(' Recipe(s)','blossom-recipe-maker').'</span>';
						}
						echo '</a></li>';
					}
					else{
		...
		$html = ob_get_clean();
        echo apply_filters( 'brm_custom_categories_widget_filter', $html, $args, $instance );
	...
	public function form( $instance ) {
		...
		<p>
			...
			<select name="<?php echo $this->get_field_name( 'taxonomy' );?>" class="brm-taxonomy-selector widefat" id="<?php echo $this->get_field_id( 'taxonomy' ); ?>" >
		...
		<p>
			...
			<?php if( isset( $taxonomy ) && $taxonomy != '' )
			{
				...
				{
					?>
					<select name="<?php echo $this->get_field_name( 'categories[]' );?>" class="brm-cat-select brm-categories-select-<?php echo esc_attr( absint( $ran ) );?>" id="brm-categories-select-<?php echo esc_attr( absint( $ran ) );?>" multiple style="width:350px;" tabindex="4">
					  	<?php
					  	$cats = get_categories('taxonomy='.$taxonomy);
					  						 
						foreach ( $cats as $cat ) {
							$selected = ( in_array( $cat->term_id, $categories ) ? 'selected="selected"' : '');
		...
		<span class="brm-option-side-note" class="example-text"><?php $bold = '<b>'; $boldclose = '</b>'; echo sprintf( __('To set thumbnail for categories, go to %1$sBlossom Recipes > Categories%2$s and %3$sEdit%4$s the categories.','blossom-recipe-maker'), $bold, $boldclose, $bold, $boldclose);?></span>
