...
class Brm_Recent_Recipe extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        if( $qry->have_posts() ){
                ...
                ?>
                    <li>
                        ...
                        <div class="entry-header">
                            ...
                            <h3 class="entry-title"><a <?php echo esc_attr($target); ?> href="<?php the_permalink(); ?>"><?php the_title();?></a></h3>

                            <?php if( $show_date ) { ?>
                                <div class="entry-meta">
                                    <span class="posted-on"><a <?php echo esc_attr($target); ?> href="<?php the_permalink(); ?>">
                                        <time datetime="<?php printf( __( '%1$s', 'blossom-recipe-maker' ), get_the_date('Y-m-d') ); ?>"><?php printf( __( '%1$s', 'blossom-recipe-maker' ), get_the_date('F j, Y') ); ?></time></a>
                                    </span>
                                </div>
                            <?php } ?>
            ...
            <?php
            $html = ob_get_clean();
            echo apply_filters( 'brm_recent_recipe_widget_filter', $html, $args, $instance );
