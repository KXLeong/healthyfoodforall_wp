...
<div id="blossom-recipe-tab-recipe-overview" class="inside">
	...
	<div id="br-recipe-overview">
		...
		<h1><?php _e('Overview', 'blossom-recipe-maker');?></h1>
		...
		if ( get_post_status ( $post_id ) == 'publish' ) 
        {
            ...
            <div class="br-recipe-shortcode">
            	...
            	<h4><?php _e('Recipe Shortcode', 'blossom-recipe-maker');?>
	    ...
	    <div class="br-recipe-meta-difficulty">
			...
			<label for="br_recipe_difficulty_level">
				<h4><?php _e( 'Difficulty Level', 'blossom-recipe-maker' ); ?></h4>
			</label>

			<select id="br_recipe_difficulty_level" name="br_recipe[details][difficulty_level]">
				
				<option value=""><?php _e( '--Select--', 'blossom-recipe-maker' ); ?></option>
				<?php
					foreach ($levels as $level => $name) {
						?>

						<option value="<?php echo esc_attr($level);?>"  <?php if(isset($details['details']['difficulty_level']) && ($details['details']['difficulty_level'] == $level)) { echo 'selected'; } ?>> <?php echo esc_attr( $name ) ;?> 
						</option>
						<?php
		...
		<div class="br-recipe-meta-prep-time">
			...
			<label for="br_recipe_prep_time">
				<h4><?php _e( 'Preparation Time (mins)', 'blossom-recipe-maker' ); ?></h4>
			</label>
		...
		<div class="br-recipe-meta-cook-time">
			...
			<label for="br_recipe_cook_time">
				<h4><?php _e( 'Cook Time (mins)', 'blossom-recipe-maker' ); ?></h4>
			</label>
		...
		<div class="br-recipe-meta-total-time">
			...
			<label for="br_recipe_total_time">
				<h4><?php _e( 'Total Time (mins)', 'blossom-recipe-maker' ); ?></h4>
			</label>
			<input id="br_recipe_total_time" name="br_recipe[details][total_time]" placeholder="30" min="0" type="number" value="<?php echo (isset($details['details']['prep_time']) &&(!empty($details['details']['prep_time'])) && isset($details['details']['cook_time']) && (!empty($details['details']['cook_time']))) ? (esc_attr($details['details']['prep_time']) + esc_attr($details['details']['cook_time'])) : '' ;?>" readonly/>
